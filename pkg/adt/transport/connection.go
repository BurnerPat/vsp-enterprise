...
// AdtConnection is the central abstraction for communicating with an SAP system.
// Implementations exist for direct HTTP (AdtHttpConnection) and for JCo sidecar
// proxy (AdtJcoConnection, which in turn delegates to an AdtJcoTransport).
type AdtConnection interface {
	// SendRequest sends an AdtRequest and returns an AdtResponse.
	...
	SendRequest(ctx context.Context, req *AdtRequest) (*AdtResponse, error)
